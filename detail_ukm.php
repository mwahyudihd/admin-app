...
<body x-data="redirect()">
    <div class="flex flex-col h-screen" z-0 x-data="{ sidebarOpen: true }">
        ...
        x-data="{
            ...
            fetchUKM() {
                let url = `${base}api/select_ukm.php?id=<?= $_GET['id'] ?>`;
                fetch(url)
            ...
            submitForm(baseUrl) {
                ...
                fetch(`${baseUrl}controllers/c_update_ukm.php`, {
                    method: 'POST',
                    body: formData
                })
                .then(response => response.json())
        ...
        >
            ...
            <form action="controllers/c_update_ukm.php" method="post" enctype="multipart/form-data">
                    ...
                    <div class="bg-white p-6 rounded-lg shadow-md">
                        ...
                        <div class="flex items-center justify-between mb-6">
                            <div class="flex-1 flex justify-center">
                                <img id="existingImage" alt="UKM Logo" class="w-24 h-24 rounded-full object-cover" height="400" :src="data.foto" width="400"/>
                                <img id="imagePreview" alt="Preview Image" class="w-24 h-24 rounded-full object-cover" style="display: none;" :src="data.foto" />
                            </div>
                            <?php if(isset($_GET['edit'])){ ?>
                                <input type="file" id="foto" class="hidden" hidden @change="previewImage($event)" name="foto" />
                                <a class="cursor-pointer bg-orange-500 text-white px-4 py-2 rounded" @click="document.getElementById('foto').click();">
                                    Choose File
                                </a>
                            <?php } ?>
                        </div>
                        <table class="w-full border-collapse border border-gray-300" >
                            <tbody >
                                ...
                                <tr>
                                    ...
                                    <td class="border border-gray-300 p-2">
                                        <?php if (isset($_GET['edit'])) { ?>
                                            <input type="password" x-model="data.password" name="password" class="w-full h-full">
                                        <?php } else { ?>
                        ...
                        <div class="flex justify-between mt-6">
                            <button type="submit" class="bg-green-600 text-white px-6 py-2 rounded" @click="submitForm(base)" >UPDATE</button>
                            <a class="bg-red-600 text-white px-6 py-2 rounded cursor-pointer" @click="getTo('data_ukm.php')">CANCEL</a>
                        </div>
                        <?php } ?>
                    </div>
                </main>
