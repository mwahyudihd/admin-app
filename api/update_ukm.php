...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $old_user = getUKMById($body['id']);

    if (!$old_user) {
        http_response_code(404);
        echo json_encode(['statusCode' => 404, 'message' => 'Ukm not found']);
        exit;
    }
    ...
    // Melakukan update data ukm
    $execute = updateUKM($body['id'], $data);

    if ($execute) {
        http_response_code(200);
        echo json_encode(['statusCode' => 200, 'message' => 'Ukm is updated']);
    } else {
        http_response_code(500); // Server error jika gagal update
        echo json_encode(['statusCode' => 500, 'message' => 'Ukm is not updated']);
    }
}